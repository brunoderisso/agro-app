...
export default withStyles(style) ((function NoteTechnical(props) {
    const {classes} = props;
    ...
    const [owners, setOwners] = useState(props.technical || []);
    ...
    const addUser = () => {
        ...
        cancelToken.id = tokenList.add();
        cancelToken.token = tokenList.get(cancelToken.id);
        AccountStore.updateEnvironmentUser(cancelToken, { uuid: user.value, istechnical: true, username: user.label }, responseAddUser);
    };
    ...
    const deleteUser = () => {
        ...
        cancelToken.id = tokenList.add();
        cancelToken.token = tokenList.get(cancelToken.id);
        AccountStore.updateEnvironmentUser(cancelToken, { uuid: userid, istechnical: false }, responseDeleteUser);
    };

    return (
        <Grid item xs={12} style={{ marginTop: 10 }} id="teste">
