        ...
        if (this.mounted){
            WidgetStore.getMeasure(parameters, (retrievedata) => {
                if (retrievedata.min !== undefined && retrievedata.max !== undefined && retrievedata.mean !== undefined) {
                    let t = MeasureStore.getGradientColor("SoilMoisture", retrievedata.min, retrievedata.max, retrievedata.mean)
                    if(t !== ""){
            ...
            <Card className={classes.widgetCard} classes={{ root: classes.root }} style={{backgroundColor:this.state.color}}>
                <CardContent>
                    ...
                    <Typography component="p">
                        Máxima: {(data.max) ? parseFloat(data.max).toFixed(1) : "NaN"} %
                        <br />
                        Mínima: {(data.min) ? parseFloat(data.min).toFixed(1) : "NaN"} %
                    </Typography>
                </CardContent>
            </Card>
        )
...
export default withStyles(styles)(SoilMoisture);