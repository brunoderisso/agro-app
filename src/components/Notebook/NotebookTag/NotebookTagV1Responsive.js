...
import styles from "../../../styles/Notebook/NotebookTagPage1R";
    ...
    return (
        <Grid>
                ...
                <Grid container className={classes.containerTag}>
                    <Grid item xs={1} className={classes.verticalText}>
                        {(props.tag && props.tag.producedat &&
                            (`${t('notebook.tags_manufacture')}: ` + moment(props.tag.producedat).format("DD/MM/YY") + ` - ${t('notebook.tags_expiration')}: ` + moment(props.tag.validuntil).format("DD/MM/YY") + ` - ${t('notebook.tags_lot')}: ` + (props.tag.batch)))
                    ...
                    <Grid item xs={11}>
                        <Grid container>
                            ...
                            <Grid item xs={12}>
                                <Grid container>
                                    <Grid item xs={12}>
                                        <QR value={"https://prediza.io/rastreabilidade/" + props.crop.name.replace("ç", "c") + "-" + props.crop.variety + "/?tag=" + props.tag.objectid} style={{ width: 130, height: 130, marginBottom: 10, marginTop: 10 }}></QR>
                                    </Grid>
            ...
            <Grid container className={classes.downloadButton}>
                <Button className={classes.buttonConfig} variant="contained" color="primary" onClick={tagDownload}>
                    Download
                </Button>
            </Grid>
        </Grid>
