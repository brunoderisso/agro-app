...
export default withStyles(Styles)(function FieldCard(props) {
  ...
  useEffect(() => {
    ...
    setExpanded(props.page === "ManagementProperty");
        ...
        {polygon.Points !== undefined &&
          <Grid item xs={4}>
            <Typography variant="h5" component="h2">
              <Canvas pts={polygon.Points} color={polygon.color} width="100" height="100" />
            </Typography>
          </Grid>
        }
        <Grid item xs={8}>
          <Grid container className={classes.propertyEdit}>
            {showEdit &&
              <Grid item xs={12} className={classes.cardButtons}>
                <Button variant="contained" onClick={onClickEditPoligon} style={{ background: "white", borderRadius: "1em" }} startIcon={<EditIconOut />}>{poligonEdit ? t('common.finishButton') : t('common.editButton')}</Button>
              </Grid>
            }
            {/* <Grid item xs={12}>
